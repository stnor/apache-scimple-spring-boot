...
import com.selessia.scim.framework.exception.*;
import com.selessia.scim.framework.provider.Provider;
import com.selessia.scim.framework.provider.UpdateRequest;
import com.selessia.scim.framework.provider.annotations.ScimProcessingExtension;
import com.selessia.scim.framework.provider.extensions.AttributeFilterExtension;
import com.selessia.scim.framework.provider.extensions.ProcessingExtension;
import com.selessia.scim.framework.provider.extensions.ScimRequestContext;
import com.selessia.scim.framework.provider.extensions.exceptions.ClientFilterException;
import com.selessia.scim.framework.utility.AttributeUtil;
import com.selessia.scim.framework.utility.EndpointUtil;
import com.selessia.scim.framework.utility.EtagGenerator;
...
public abstract class BaseResourceTypeResourceImpl<T extends ScimResource> implements BaseResourceTypeResource<T> {
    ...
    @Autowired
    private UpdateRequest<T> updateRequestInstance;
    ...
    public Response update(T resource, String id, AttributeReferenceListWrapper attributes, AttributeReferenceListWrapper excludedAttributes) {
        try {
            ...
            T updated;
            try {
                UpdateRequest<T> updateRequest = updateRequestInstance;
    ...
    private T processFilterAttributeExtensions(Provider<T> provider, T resource, Set<AttributeReference> attributeReferences, Set<AttributeReference> excludedAttributeReferences) throws ClientFilterException {
        ...
        if (annotation != null) {
            ...
            for (Class<? extends ProcessingExtension> class1 : value) {
                ProcessingExtension processingExtension = null; //TODO CDI.current().select(class1).get();
